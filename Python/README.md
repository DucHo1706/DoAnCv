...
- HTTP 200 từ router/Gemini chưa mặc nhiên là thành công: `finish_reason=length/max_tokens`, Markdown, tag gợi ý job chưa đóng hoặc meta-reasoning định dạng rõ ràng sẽ bị xem là phản hồi chưa hoàn chỉnh và chuyển sang model kế tiếp trong ngân sách. Chatbot cho phép tối đa 4.096 token để không cắt phần kết khi model dùng token suy luận ẩn, nhưng prompt vẫn yêu cầu câu trả lời ngắn; câu ngắn đã đóng cấu trúc vẫn được giữ.
