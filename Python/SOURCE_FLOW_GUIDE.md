...
- Python cho phép tối đa 4.096 output token vì một số combo tính cả token suy luận ẩn vào giới hạn; prompt vẫn bắt trả lời ngắn nên đây là khoảng trống để hoàn tất, không phải yêu cầu sinh câu dài. Hệ thống vẫn chỉ giữ tối đa 10 lượt lịch sử và ngữ cảnh job có chọn lọc.
- `gemini_service` không coi mọi HTTP 200 là thành công: `finish_reason=length/max_tokens` từ cả router lẫn Gemini trực tiếp bị loại. `scoring_service._is_complete_chat_response` chỉ chặn dấu hiệu cắt chắc chắn như cặp `**`, khối code, tag `[RECOMMEND_JOB: ...]` chưa đóng hoặc meta-reasoning định dạng rõ ràng; câu trả lời ngắn nhưng hoàn chỉnh vẫn hợp lệ. Model kế tiếp được thử trong cùng ngân sách thay vì trả đoạn bị cụt cho người dùng.
