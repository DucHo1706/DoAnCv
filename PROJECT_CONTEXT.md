...
| P1-04 | Chatbot fast-path và kiểm chứng gợi ý job | ĐANG LÀM | Python AI, 9Router/Gemini, catalog job | Router dùng danh sách model nhanh riêng, lỗi giữ HTTP 503, thẻ job đối chiếu SQL; unit/build đạt, chờ đo và smoke HTTPS sau deploy |
...
- 2026-08-25: Chatbot dùng fast-path 9Router riêng thay vì đi qua toàn bộ chuỗi model phân tích CV; ngân sách router/direct hữu hạn, lỗi provider là HTTP 503 và không được lưu như tin nhắn AI thành công. Thẻ gợi ý job phải được backend đối chiếu ID và chuẩn hóa metadata từ SQL. Tác động: giảm thời gian chờ do model chậm, giữ ngữ cảnh job chính xác hơn và không biến nội dung model bịa thành liên kết tuyển dụng hợp lệ; số đo thực tế vẫn phụ thuộc provider đang hoạt động.
