...
export function useTalentPoolDetail() {
  ...
  const fetchInviteSuggestions = async () => {
    ...
    try {
      ...
      const firstVisibleJob = data.suggestedJobs.find((job) => Number(job.matchScore || 0) > 0);
      if (firstVisibleJob) {
  ...
  const visibleSuggestedJobs = useMemo(() => {
    return (inviteSuggestion?.suggestedJobs || []).filter((job) => Number(job.matchScore || 0) > 0);
  }, [inviteSuggestion]);
  ...
  const handleSendInvite = async () => {
    ...
    try {
      const selectedSuggestedJob = visibleSuggestedJobs.find((job) => job.jobId === selectedJobId);
