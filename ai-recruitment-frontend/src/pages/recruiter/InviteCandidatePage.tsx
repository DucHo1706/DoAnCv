...
export default function InviteCandidatePage() {
  ...
  const sendInvite = () => {
    ...
    message.success(`Đã gửi lời mời ứng tuyển vị trí [${job?.position?.name || 'này'}] tới ${candidate?.name}!`);
    navigate("/recruiter/talent-pool");
  };

  if (loading) return <div style={{ textAlign: "center", padding: "100px 0" }}><Spin size="large" /></div>;
  if (!candidate) return <PageContainer title="Không tìm thấy ứng viên"><Button onClick={() => navigate(-1)}>Quay lại</Button></PageContainer>;
  ...
  return (
    <PageContainer
      ...
      extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate(-1)}>Quay lại Talent Pool</Button>}
    >
      <Row gutter={24}>
        <Col xs={24} lg={16}>
          <Card style={{ borderRadius: 12 }}>
            ...
            <div style={{ padding: 16, background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, marginBottom: 24 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                ...
                <Text strong style={{ color: "#16a34a", fontSize: 16 }}>AI Phân tích & Đề xuất</Text>
              </div>
              <Text>Hệ thống tự động quét <b>{jobs.length}</b> chiến dịch đang mở và tìm thấy 2 vị trí phù hợp nhất:</Text>
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                {suggestedJobs.map(job => (
                  <div key={job.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", padding: "8px 12px", borderRadius: 6, border: "1px solid #d9f9e6" }}>
                    <div>
                      <Text strong>{job.position?.name || 'Vị trí IT'} ({job.branch?.name})</Text>
                      <div style={{ fontSize: 13, color: "#8c8c8c", marginTop: 4 }}><FireOutlined style={{ color: "#faad14" }}/> Độ phù hợp: <Text strong type="success">{job.matchScore}%</Text></div>
                    </div>
                    <Button size="small" type={selectedJobId === job.id ? "primary" : "default"} onClick={() => setSelectedJobId(job.id)}>
                      {selectedJobId === job.id ? "Đang chọn" : "Chọn job này"}
                    </Button>
                  </div>
                ))}
                {suggestedJobs.length === 0 && <Text type="secondary">Chưa có công việc nào đang mở.</Text>}
              </div>
            </div>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <Button size="large" type="primary" icon={<SendOutlined />} onClick={sendInvite}>Gửi Lời Mời Ngay</Button>
            </div>
          </Card>
        </Col>
        ...
        <Col xs={24} lg={8}>
            <Card title="Thông tin tóm tắt" style={{ borderRadius: 12 }}>
              <Paragraph><Text type="secondary">Email:</Text> <br/><Text strong>{candidate.email}</Text></Paragraph>
              <Paragraph><Text type="secondary">Công việc nộp gần nhất:</Text> <br/><Text strong>{candidate.lastApplied}</Text></Paragraph>
              <Paragraph><Text type="secondary">Trạng thái:</Text> <br/><Text strong type="success">{candidate.status}</Text></Paragraph>
              <Paragraph><Text type="secondary">Điểm AI cao nhất từng đạt:</Text> <br/><Text strong style={{ color: '#faad14' }}>{candidate.maxAiScore} / 100</Text></Paragraph>
            </Card>
        </Col>
      </Row>
    </PageContainer>
