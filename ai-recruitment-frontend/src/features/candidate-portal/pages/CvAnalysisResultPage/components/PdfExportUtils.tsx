...
const PdfExportUtils: React.FC<PdfExportUtilsProps> = ({
  ...
  const redFlagSuspicions = safeList(score.red_flag_suspicions);
      ...
      <div aria-hidden="true" style={{ position: "fixed", left: "-12000px", top: 0, width: 794 }}>
        <article ref={printRef} className="pdf-report">
          <style>{`
            ...
            .pdf-list{margin:0;padding-left:17px}.pdf-list li{margin:5px 0}.pdf-tags{display:flex;flex-wrap:wrap;gap:6px}.pdf-tag{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;padding:3px 7px;border-radius: 8px}
          ...
          <section className="pdf-section pdf-keep">
            ...
            {redFlags.length > 0 && <div style={{ marginTop: 16 }}><h3>Red flag cần xác minh</h3>{redFlags.map((flag, i) => <div className="pdf-note pdf-keep" key={i}><strong>{flag.title || "Điểm cần làm rõ"}</strong><br />{flag.description || ""}{flag.evidence_text ? <><br /><strong>Đoạn trích trong CV:</strong> “{flag.evidence_text}”<br /><span>Đây là thông tin ứng viên tự khai, chưa được xác minh với nguồn bên ngoài.</span></> : null}</div>)}</div>}
            {redFlagSuspicions.length > 0 && <div style={{ marginTop: 16 }}><h3>Dấu hiệu AI đề xuất – chưa đối chiếu</h3><div className="pdf-empty" style={{ marginBottom: 10 }}>Các mục này không có đoạn trích gần-nguyên-văn, không tham gia chấm điểm và không phải kết luận ứng viên gian dối.</div>{redFlagSuspicions.map((flag, i) => <div className="pdf-note pdf-keep" key={i}><strong>{flag.title || "Nội dung AI đề xuất kiểm tra"}</strong><br />{flag.description || ""}</div>)}</div>}
          </section>
