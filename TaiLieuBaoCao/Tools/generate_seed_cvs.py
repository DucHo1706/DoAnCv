...
def create_beautiful_pdf(data, output_path):
    ...
    class CVPdf(FPDF):
        ...
        def section_header(self, title):
            ...
            self.cell(190, 7, title.upper(), new_x="LMARGIN", new_y="NEXT")
            # Đường kẻ
        ...
        def info_row(self, label, value):
            ...
            self.cell(155, 5, value, new_x="LMARGIN", new_y="NEXT")
        
        def bullet(self, text):
            """Bullet point với indent nhỏ"""
            self.set_font(self.font_name, "", 9)
            self.set_x(self.l_margin + 4)
            self.multi_cell(186, 5, f"•  {text}", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.cell(190, 10, d["name"], new_x="LMARGIN", new_y="NEXT", align="C")
    
    # Dòng thông tin liên hệ ngắn gọn
    pdf.set_font(pdf.font_name, "", 9)
    pdf.set_text_color(100, 100, 100)
    contact = f"{d['email']}  |  {d['phone']}  |  {d['address']}"
    pdf.cell(190, 5, contact, new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(2)
    
    # ===== MỤC TIÊU NGHỀ NGHIỆP =====
    pdf.section_header("Mục tiêu nghề nghiệp")
    pdf.set_font(pdf.font_name, "", 9)
    pdf.multi_cell(190, 5, d["objective"], new_x="LMARGIN", new_y="NEXT")
    
    # ===== HỌC VẤN =====
    pdf.section_header("Học vấn")
    pdf.set_font(pdf.font_name, "B", 10)
    pdf.multi_cell(190, 5, d["univ"], new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(pdf.font_name, "", 9)
    pdf.multi_cell(190, 5, f"Chuyên ngành: {d['major']}  |  Bằng: {d['degree']}  |  Thời gian: {d['grad_year']-4} - {d['grad_year']}", new_x="LMARGIN", new_y="NEXT")
    pdf.multi_cell(190, 5, f"GPA: {d['gpa']}/4.0  |  Xếp loại: {d['gpa_class']}", new_x="LMARGIN", new_y="NEXT")
    ...
    # Công ty 1
    pdf.set_font(pdf.font_name, "B", 10)
    pdf.multi_cell(190, 6, f"{d['company1'][0]} ({d['company1'][1]})", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(pdf.font_name, "", 9)
    pdf.set_text_color(80, 80, 80)
    pdf.multi_cell(190, 5, f"{d['title1']}  |  {d['period1']}", new_x="LMARGIN", new_y="NEXT")
    ...
    # Công ty 2
    pdf.set_font(pdf.font_name, "B", 10)
    pdf.multi_cell(190, 6, f"{d['company2'][0]} ({d['company2'][1]})", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(pdf.font_name, "", 9)
    pdf.set_text_color(80, 80, 80)
    pdf.multi_cell(190, 5, f"{d['title2']}  |  {d['period2']}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(30, 30, 30)
    ...
    pdf.multi_cell(190, 5, f"1. {d['proj1'][0]}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(pdf.font_name, "", 9)
    ...
    pdf.set_font(pdf.font_name, "B", 9)
    pdf.multi_cell(190, 5, f"2. {d['proj2'][0]}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(pdf.font_name, "", 9)
